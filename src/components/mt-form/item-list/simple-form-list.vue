<template>
	<SelectBox ref="SelectBox" @cancel="_listCancel" @submit="_listSubmit">
		<template slot="picker">
			<view class="simple-form-list-main" v-if="chooseList && chooseList.length">
				<scroll-view scroll-y="true" @scrolltolower="lowerEvent" :lower-threshold="1">

					<view class="scroll-view-item" v-for="(item, index) in chooseList" :key="index"
						:class="{'selected': selectedId.indexOf(item[showValue.key]) > -1}" @tap="tapItem(item)">

						<view class="briefBox">
							<i class="iconfont icon-position"></i>
							<view class="brief">
								<view>{{ item[showValue.value] }}</view>
								<text>{{ item[showValue.brief] }}</text>
							</view>
						</view>

						<view class="chosed">
							<i class="iconfont icon-xuanzhong"
								v-show="selectedId.indexOf(item[showValue.key]) > -1"></i>
						</view>
					</view>
			...
			<view class="simple-form-list-empty" v-else>
...
<script>
	...
	export default {
		...
		methods: {
			...
			tapItem(item) {
				this.selectedId = [item[this.showValue.key]];
				this.selectedText = [item[this.showValue.value]];
				this.allSelected = [item];
			}
		}
	};
</script>
